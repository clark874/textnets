...
class Corpus:
    ...
    def custom_tokenized(
            ...
            """Return tokenized version of corpus using custom dictionary in tidy format.

            Parameters
            ----------
            custom_dict : list of str
                Custom dictionary to use for tokenization.
            remove : list of str, optional
                Additional tokens to remove.
            stem : bool, optional
                Return token stems (default: True).
            ...
            func = compose(
                ...
                _remove_numbers if remove_numbers else identity,
                _remove_punctuation if remove_punctuation else identity,
                partial(_custom_tokenize, custom_dict=custom_dict)
            )
            tt = self._make_tidy_text(func)
            return _tf_idf(tt, sublinear)
...
def _custom_tokenize(doc: Doc, custom_dict: list[str]) -> list[str]:
    """Tokenize the document using jieba with a custom dictionary for Chinese text."""
    # Add custom dictionary to jieba
    for word in custom_dict:
        jieba.add_word(word)
    
    # Convert the doc to a single string
    text = ''.join([token.text for token in doc])
    
    # Perform jieba tokenization
    tokens = jieba.lcut(text)
    
    # Filter tokens based on custom dictionary
    final_tokens = [token if token in custom_dict else token for token in tokens]
...
class TidyText(LiteFrame):
    ...
    def to_dataframe(self):
        """将 TidyText 对象转换为 Pandas DataFrame。"""
        return self._data
